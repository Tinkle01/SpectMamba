...
def DSL(pred_u_self, eta=0.95,max_class_num=15):
    with torch.no_grad():
        ...
        dsl_y_star = y_star.permute(1, 2, 0)
...
def contrastive_loss4(query1, predition1, query2, predition2, weights, linear_wieght, temperature=0.5, m=0.9):
    ...
    neg_sim = pos_sim.clone() *(1-mask) 

    #torch.fill_diagonal_(neg_sim, -float('inf')) 

    positive_confid = pred2.permute(0,2,1)  
    tch_value, tch_index = torch.max(positive_confid, dim=2)  
    #print(tch_value)

    anchor_confid = pred1.permute(0,2,1)  
    batch_indices = torch.arange(batch_size).unsqueeze(1)  
    query_indices = torch.arange(query_num).unsqueeze(0) 
    stu_value = anchor_confid[batch_indices, query_indices, tch_index] 

    update_weight = torch.sum(tch_value) / (batch_size * query_num)
    #print(update_weight)

    global_weight = m * weights + (1-m) * update_weight  
    with torch.no_grad():
    ...
    pos_sim = pos_sim_diag * valid_mask.unsqueeze(2).expand(-1, -1, query_num) 
